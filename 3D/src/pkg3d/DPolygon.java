...
public class DPolygon {
       ...
       public DPolygon(double [] x, double[] y, double[]z, Color c)
    {
       ...
       void updatePolygon()
       {
           ...
           {
               newX[i] = dx+ DDD.ScreenSize.width/2 +50* Calculator.CalculatePositionX(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);
               newY[i] = dy+ DDD.ScreenSize.height/2 +50* Calculator.CalculatePositionY(Screen.ViewFrom, Screen.ViewTo, x[i], y[i], z[i]);     
           }
           
           
           Screen.DrawablePolygons[poly] = new PolygonObject(newX, newY, c);
           Screen.DrawablePolygons[poly].AvgDist = GetDist();
           Screen.setDefaultLocale(Locale.UK);
