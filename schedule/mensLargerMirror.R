...
######################################
#Mirror of 2023 season, Using same dates, but since the order is the same, you can just change the dates at the end in your exported excel file
...
# Mirror that eliminates teams playing each other at the same location two years in a row
for(home in namesOfTeamsWithoutBye){
  for(visitor in namesOfTeamsWithoutBye){
    for(playDate in namesOfPlayDates){
      if(home != visitor){
        if(matchUps2023[home,visitor] == "FALSE"){
        newVariable = paste("x", home, visitor, playDate, wday(playDate), sep = ".")
        newVariable1 = paste("x", "Bye", visitor, playDate, wday(playDate), sep = ".")
        namesOfVariables= c(namesOfVariables, newVariable)
         newVariable2 = paste("x", home, "Bye", playDate, wday(playDate), sep = ".")
        namesOfVariables = c(namesOfVariables, newVariable1)
         namesOfVariables = c(namesOfVariables, newVariable2)# tack new one onto it
        }
      }
    }
  }
...
write.lp(myFirstLP,'ProjectMensAllConstraintsMirror.mps',type='mps')
...
optimalSolutionVector = coinOrCbcSolutionParser("ProjectMenMirror.txt", namesOfVariables)
